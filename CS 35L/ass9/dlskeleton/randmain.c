...
main (int argc, char **argv)
{
    ...
    if(rdrand_supported())
    {
        ...
        err=dlerror();
        if(dl_handle==NULL){
            printf("Can't open library randlibhw.so %s \n",err);
            exit(1);
        }
        ...
        if(err!=NULL){
            ...
            printf("Can't open hardware funciton %s \n",err);
            exit(1);
        }
        ...
        err=dlerror();
        if(dl_handle==NULL){
            printf("Can't open the library %s \n",err);
            exit(1);
        ...
        if(err!=NULL){
            printf("Can't open software function %s \n",err);
            exit(1);
        }
