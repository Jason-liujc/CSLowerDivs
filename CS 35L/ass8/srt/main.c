...
float scaled_color[width][height][3]; // declare after the enums
...
void * ThreadFunction(void* arg)
{
    ...
    for( int px=*(int*)arg; px<width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            // changing here because it was used in both functions
...
//    }
    ...
    {
        id[k]=k;
        succ=pthread_create(&threads[k],NULL,ThreadFunction,&id[k]);
        if(succ)
        {
