...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    {
        int loc=unsorted;  //this part come from the book.
        
        Sensor Next=s[unsorted];
        
        for (;(loc>0)&&compareSensor(Next, s[loc-1]);--loc)
        {
            s[loc]=s[loc-1];
...
int main()
{
    ...
    // destruction time.
    {
        ...
        vector<Sensor*> pointer;
      
       
        for (int i = 0; i < auxSensors.size(); i++)
        {
            pointer.push_back(&auxSensors[i]);
        }
        
        //sensorptr points to auxsensor's elements.
        
        sort(pointer.begin(), pointer.end(),compareSensorPtr);
