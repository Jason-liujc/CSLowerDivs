...
{
    int counter2=0;
    Multiset ms2(DEFAULT_MAX_ITEMS);  //create a ms2 object to figure out the unique size.
...
void Multiset::calculateUnique()
{
    ...
    int counter2=0;
    Multiset ms10(DEFAULT_MAX_ITEMS);  //create a ms2 object to figure out the unique size.
    ...
    for (int i=0; i<ms10.size()*2; i++)
    {
        if (ms10.count(set[i])!=1)
        {
            ms10.erase(set[i]);
